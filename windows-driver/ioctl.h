...
#define IOCTL_LAMPYRIS_AUTHENTICATE \
    CTL_CODE(FILE_DEVICE_LAMPYRIS, LAMPYRIS_FUNC_AUTHENTICATE, METHOD_BUFFERED, FILE_WRITE_ACCESS)
...
typedef struct _LAMPYRIS_AUTH_PAYLOAD {
    BYTE Token[32];
} LAMPYRIS_AUTH_PAYLOAD, *PLAMPYRIS_AUTH_PAYLOAD;
